<template>
  <q-layout style="width: 100%; height: 100%; min-height:calc(100vh - 50px);"
            view="hHh Lpr fff">
    <div class="row">
      <div class="col">
            ...
            <draggable :list="sourceFields" :clone="createField" :options="sourceOptions"
              ...
              <div class="source-field" v-for="(sourceField, idx) in sourceFields" :key="idx">
                <q-btn type="button" size="12px" dense align="left" class="btn-fixed-width"
                       @click="onAddFieldClick(sourceField.type)" :icon="sourceField.icon"
                       :label="sourceField.label" v-if="sourceField.type !== ''"/>
              </div>
            </draggable>
          </q-tab-panel>
            ...
            <draggable v-model="fields" @change="onChange" :options="destinationOptions"
              ...
              <div class="editable-element-container" v-for="(field, idx) in fields" :key="idx">
                ...
                <div class="editable-element-action-buttons">
                  <q-btn class="editable-element-button" v-if="isSelectedForEdit(idx)"
                         @click="deleteField(idx)" color="red" icon="delete" round size="xs">
                    <q-tooltip>Delete this field</q-tooltip>
                  </q-btn>
                  <q-btn class="editable-element-button" v-if="isSelectedForEdit(idx)"
                         @click="duplicateField(idx)" color="secondary" icon="file_copy" round
                         size="xs">
