  ...
  <div>
    <q-card>
      ...
      <q-card-section v-if="show">
        <center>
          <p class="text-h6">Produk Masuk</p>
          <p>Koperasi {{ res.namaKoperasi }}</p>
          <p>{{ getDate(dateFrom) }} s/d {{ getDate(dateTo) }}</p>
          <table>
            <tr>
              <td><b>Total Produk Masuk</b></td>
              <td>:</td>
              <td>{{ res.totProdMasuk }}</td>
            </tr>
          </table>
        </center>
      </q-card-section>
  ...
  export default {
        ...
        columns: [
          {
            ...
            label: 'Tanggal Masuk',
            align: 'center',
            field: row => this.getDate(row.tanggal_masuk),
            sortable: true,
          },
        ]
      }
    ...
    methods: {
      ...
      download() {
        ...
        this.$q.loading.show()
        this.$http.post('/api/getlaporanpemasukanproduk/download', {
          ...
          .then((res) => {
            this.downloadFile(res, 'PemasukanProduk')
            this.$q.loading.hide()
          })
          .catch(() => {
            this.$q.notify({
      ...
      showLaporan() {
        ...
        this.$q.loading.show()
        this.$http.post('/api/getlaporanpemasukanproduk', {
          dateFrom: this.dateFrom,
          dateTo: this.dateTo
        }, {
          headers: this.$auth.getHeader()
